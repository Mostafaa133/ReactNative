...
export default function ToDoList() {
  return (
    <TasksContextProvider test="mostafa">
...
function TasksProviderComp() {
  ...
  const { AllTasks, postTask, updateTask, deleteTask } =
    ...
    <View style={{ marginHorizontal: 15 }}>
      <View style={[styles.flex]}>
        ...
        <Pressable
          ...
          style={[styles.btnContainer, { opacity: task.length > 0 ? 1 : 0.5 }]}
          onPress={createTask}
        >
          <Text>Add</Text>
        </Pressable>
          ...
          renderItem={(task) => (
            <View style={styles.flex}>
              <View style={styles.flex}>
                ...
                <Text
                  style={{
                    textDecorationLine: task.item.done
                      ? "line-through"
                      : "none",
                  }}
                >
              ...
              <Pressable
                disabled={!task.item.done}
                style={[
                  styles.btnContainer,
                  { opacity: task.item.done ? 1 : 0.7 },
                ]}
